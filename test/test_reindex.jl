@testitem "reindex" begin
    using SpacePhysicsMakie: reindex
    A = [10, 20, 30, 40, 50]
    ...
    @testset "reindex nearest neighbor" begin
        ...
        result_tol = reindex(A, old_coords, [1.5, 6.0]; method = :nearest, tolerance = 0.4, fill_value = -1)
        @test result_tol[1] == -1  # 1.5 is 0.5 from both 1.0 and 2.0, exceeds 0.4
        @test result_tol[2] == -1  # 6.0 exceeds tolerance
    end
...
@testitem "reindex multi-dimensional arrays" begin
    using SpacePhysicsMakie: reindex
...
@testitem "reindex edge cases" begin
    using SpacePhysicsMakie: reindex
...
@testitem "reindex DimArray" begin
