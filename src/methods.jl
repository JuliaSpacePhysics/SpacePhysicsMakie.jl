...
Set axis limits, handling different time formats.
Supports DateTime, Unitful, and numeric time values.
"""
...
function default_orientation(position)
    return position in (Top(), Bottom()) ? :horizontal : :vertical
end
...
function add_legend!(ap::Makie.AxisPlot; kwargs...)
    ax = ap.axis
    gp = gridposition(ax)
    return add_legend!(gp, ax; kwargs...)
end
...
function add_legend!(p::PanelAxesPlots; kwargs...)
    ...
    return add_legend!(p.pos, ax; kwargs...)
end
...
function add_labels!(f = current_figure(); allowedblocks = Union{Axis, Axis3, PolarAxis}, kwargs...)
    ...
    return add_labels!(unique(layouts); kwargs...)
end
